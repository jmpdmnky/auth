...
# Are we buildiong or destroying?
...
def create_resources(template):
    ...
    for function in lambda_functions:
        ...
        args = dict_select(attr, ['FunctionName', 'Runtime', 'Role', 'Handler', 'Code', 'Timeout', 'MemorySize', 'Publish'])
    ...
    for pool in user_pools:
        ...
        args = dict_select(attr, ['PoolName', 'Policies', 'LambdaConfig', 'UsernameConfiguration','Schema'])
...
def modify_resources(template):
    ...
    for pool in user_pools:
        name, attr = template_item(pool)
        args = dict_select(attr, ['ClientName', 'GenerateSecret'])
...
def destroy_resources(resources, template):
    ...
    for name in to_delete:
        # for now, only have aws managed, so just remove from list
        live_resources.pop(name, None)
